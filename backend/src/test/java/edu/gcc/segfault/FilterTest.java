...
class FilterTest {
    ...
    void filterByStartTime() {
        // 15:30 → should include only acctTR
        ...
        assertFalse(result.contains(astrCourse), "19:00 start should be excluded");
    ...
    @Test
    void filterByMultipleStartTimesORLogic() {
        filter.setStartTimes(new LocalTime[]{LocalTime.of(12, 0), LocalTime.of(10, 0)});
        Set<Course> result = filter.applyFilters(allCourses);

        assertTrue(result.contains(compCourse),  "12:00 start matches filter");
        assertTrue(result.contains(acctMWF),     "10:00 start matches filter");
        assertFalse(result.contains(acctTR),     "15:30 start should be excluded");
        assertFalse(result.contains(astrCourse), "19:00 start should be excluded");
    }
    ...
    void filterByEndTime() {
        ...
        assertFalse(result.contains(acctMWF),     "Ends 10:50, should be excluded");
    ...
    @Test
    void filterByMultipleEndTimesORLogic() {
        filter.setEndTimes(new LocalTime[]{LocalTime.of(10, 50), LocalTime.of(12, 50)});
        Set<Course> result = filter.applyFilters(allCourses);

        assertTrue(result.contains(acctMWF),     "10:50 end matches filter");
        assertTrue(result.contains(compCourse),  "12:50 end matches filter");
        assertFalse(result.contains(acctTR),     "16:45 end should be excluded");
        assertFalse(result.contains(astrCourse), "21:30 end should be excluded");
    }
