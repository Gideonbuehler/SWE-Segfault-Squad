...
function SearchPage({
    ...
    const runFilter = async () => {
        const hasFilters =
            filters.department || filters.professor || filters.credits ||
            filters.days.length > 0 || filters.startTime || filters.endTime;
        if (!hasFilters) { await clearFilter(); return; }
        ...
        const response = await fetch(`/api/filterResults/${query}/filter?${params}`, { method: "POST" });
        if (response.ok) {
            const data = await response.json();
            setExpandedDescriptions({});
            setResults(sortByCourseCode(Array.from(data)));
    ...
    useEffect(() => {
        const debounceTimer = setTimeout(async () => {
            ...
            const hasFilters =
                filters.department || filters.professor || filters.credits ||
                filters.days.length > 0 || filters.startTime || filters.endTime;
            if (hasFilters) await runFilter();
        }, 300);
        return () => clearTimeout(debounceTimer);
    }, [query]);

    useEffect(() => {
        const debounceTimer = setTimeout(() => { runFilter(); }, 300);
        return () => clearTimeout(debounceTimer);
    }, [filters]);

    const addCourse = async (courseCode, semester) => {
        const response = await fetch(`/api/mySchedule/add/${courseCode}/${semester}`, { method: "POST" });
        if (response.ok) {
    ...
    const removeCourse = async (courseCode, semester) => {
        const response = await fetch(`/api/mySchedule/remove/${courseCode}/${semester}`, { method: "DELETE" });
        if (response.ok) {
    ...
    const feelingLucky = async () => {
        ...
        const data = await response.json();
        const pool = selectedSemester ? data.filter(c => c.semester === selectedSemester) : data;
        const notAdded = pool.filter(c => !schedule.some(s => s.courseCode === c.courseCode && s.semester === c.semester));
        if (!notAdded.length) { alert("No available courses!"); return; }
        const shuffled = [...notAdded].sort(() => Math.random() - 0.5);
        for (const candidate of shuffled) {
            const res = await fetch(`/api/mySchedule/add/${candidate.courseCode}/${candidate.semester}`, { method: "POST" });
            if (res.ok) { await fetchSchedule(); alert(`Added ${candidate.courseCode}`); return; }
        }
        ...
        <div>
            ...
            <div style={{ display: "flex", gap: "10px", marginTop: "10px" }}>
                ...
                <button
                    ...
                    style={{ backgroundColor: "#7c3aed", color: "white", border: "none", padding: "8px 16px", borderRadius: "4px", cursor: "pointer" }}
                >
                    🎲 I'm Feeling Lucky
                </button>
            </div>
            ...
            <div className="card" style={{ marginBottom: "10px" }}>
                ...
                <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
                    <label>
                        Department:
                        <input type="text" placeholder="e.g. COMP" value={filters.department}
                            onChange={(e) => setFilters({ ...filters, department: e.target.value })}
                            style={{ marginLeft: "8px" }} />
                    </label>
                    <label>
                        Professor:
                        <input type="text" placeholder="e.g. Wolfe" value={filters.professor}
                            onChange={(e) => setFilters({ ...filters, professor: e.target.value })}
                            style={{ marginLeft: "8px" }} />
                    </label>
                    <label>
                        Credits:
                        <input type="number" placeholder="e.g. 3" value={filters.credits}
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
                    ...
                    <tbody>
                        {filteredResults.map((course, index) => {
                            ...
                            const rowColor = darkMode ? "#f9fafb" : "#111827";
                            return (
                                <tr key={index} style={{ borderBottom: "1px solid #e5e7eb", backgroundColor: rowBg, color: rowColor, opacity: course.isOpen ? 1 : 0.6 }}>
                                    <td style={{ padding: "10px" }}><b>{course.courseCode}</b></td>
                                    <td style={{ padding: "10px" }}>
                                        <div>{course.courseName}</div>
                                        <div style={{ marginTop: "4px", color: "#2563eb", fontSize: "13px" }}>
                                            {isExpanded || !hasLongDescription
                                                ? description || "No description available."
                                                : description.slice(0, DESCRIPTION_PREVIEW_LENGTH) + "..."}
                                        </div>
                                        {hasLongDescription && (
                                            <button type="button" onClick={() => toggleDescription(courseKey)}
                                                style={{ marginTop: "4px", border: "none", background: "none", color: "#2563eb", padding: 0, cursor: "pointer" }}>
                                    ...
                                    <td style={{ padding: "10px" }}>
                                        {course.isOpen ? (
                                            schedule.some(c => c.courseCode === course.courseCode && c.semester === course.semester) ? (
                                                <button onClick={() => removeCourse(course.courseCode, course.semester)}
                                                    style={{ backgroundColor: "#dc2626", color: "white", border: "none", padding: "6px 12px", borderRadius: "4px", cursor: "pointer" }}>
                                                    Remove
                                                </button>
                                            ) : (
                                                <button onClick={() => addCourse(course.courseCode, course.semester)}
                                                    style={{ backgroundColor: "#1d54a1", color: "white", border: "none", padding: "6px 12px", borderRadius: "4px", cursor: "pointer" }}>
                                                    Add
                                                </button>
                                            )
                                        ) : null}
